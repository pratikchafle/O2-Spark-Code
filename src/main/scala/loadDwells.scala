  ...
  def main(args: Array[String]) {
    ...
    try {
      ...
      val empirixDF = spark.read.table(s"$schEmpDB.$empirixConsolidated").filter($"year" === s"$year" && $"month" === s"$month" && $"day" === s"$day")
        ...
        .withColumn("cell_id", coalesce($"start_source_cell_id", $"end_source_cell_id").substr(-(length(coalesce($"start_source_cell_id", $"end_source_cell_id").cast(StringType)) - 1), length(coalesce($"start_source_cell_id", $"end_source_cell_id").cast(StringType)) - 1))
        .select($"user_id", $"date_dt", $"start_dt", $"cell_id", $"year", $"month", $"day")


      val cellsDF = spark.read.table(s"$schVisitDB.$cells").select($"cell_id").distinct()
      ...
      if (jobStatus == "Completed") {
        ...
        conn.close()
        spark.stop()
        val appIDTest = spark.sparkContext.applicationId
      }
    }
